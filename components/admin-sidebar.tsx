...
export const AdminSidebar = memo(function AdminSidebar({ isOpen, onToggle }: AdminSidebarProps) {
  ...
  const tabs = [
    ...
    { id: "receipts", label: "Comprovantes", icon: FileCheck, href: "/admin/comprovantes" },
    { id: "payments", label: "Pagamentos", icon: Banknote, href: "/admin/pagamentos" },
    { id: "codes", label: "Códigos de Acesso", icon: KeyRound, href: "/admin/codigos" },
    { id: "announcements", label: "Avisos", icon: Bell, href: "/admin/avisos" },
  ]
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 space-y-1 p-4 overflow-y-auto overscroll-contain">
          <p className="mb-3 px-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">Administração</p>
          {tabs.map((tab) => {
            const Icon = tab.icon
            ...
            return (
              ...
              >
                ...
                <span>{tab.label}</span>
                {isActive && <ChevronRight className="ml-auto h-4 w-4 text-pink-400" />}
              </Link>
            )
          })}
        </nav>

        <div className="border-t border-white/5 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          <div className="rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 p-3">
            <div className="mb-1 flex items-center gap-2">
              <Shield className="h-4 w-4 text-pink-400" />
              <p className="text-xs font-medium text-pink-400">Administrador</p>
            </div>
