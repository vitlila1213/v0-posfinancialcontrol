...
import { X, Wallet, Check, AlertTriangle, CreditCard, QrCode, Loader2, FileText } from "lucide-react"
...
export function RequestWithdrawalModal({ open, onOpenChange, availableBalance }: RequestWithdrawalModalProps) {
  ...
  const [pixKeyType, setPixKeyType] = useState<PixKeyType>("cpf") // Added PIX key type state
  const [pixOwnerName, setPixOwnerName] = useState(profile?.full_name || "") // Added PIX owner name state
  ...
  const handleSubmit = async () => {
    ...
    try {
      await requestWithdrawal({
        ...
        pixKeyType: paymentMethod === "pix" ? pixKeyType : undefined, // Pass PIX key type
        pixOwnerName: paymentMethod === "pix" ? pixOwnerName : undefined, // Pass PIX owner name
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <motion.div
              ...
              className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 bg-slate-900/95 p-4 shadow-2xl backdrop-blur-xl pointer-events-auto sm:p-6"
            >
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      Solicitar {numericAmount > 0 && formatCurrency(numericAmount)}
                    </>
                  )}
                </Button>
              </div>
